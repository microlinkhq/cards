      ...
      >
        <MQL
          ...
          options={{
            ...
            data: {
              ...
              website: {
                selector: 'header.pagehead [itemprop=url]:not([href*="twitter.com/"])'
              },
              twitter: {
        ...
        >
          {(payload) => {
            ...
            const headerStats = (data.location || data.website || data.twitter || data.email)
              ? [
                  {
                    icon: 'place',
                    value: data.location
                  ...
                  {
                    svg: <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 273.5 222.3' width='16' height='16' style={{ display: 'block' }}><path d='M273.5 26.3a109.77 109.77 0 0 1-32.2 8.8 56.07 56.07 0 0 0 24.7-31 113.39 113.39 0 0 1-35.7 13.6 56.1 56.1 0 0 0-97 38.4 54 54 0 0 0 1.5 12.8A159.68 159.68 0 0 1 19.1 10.3a56.12 56.12 0 0 0 17.4 74.9 56.06 56.06 0 0 1-25.4-7v.7a56.11 56.11 0 0 0 45 55 55.65 55.65 0 0 1-14.8 2 62.39 62.39 0 0 1-10.6-1 56.24 56.24 0 0 0 52.4 39 112.87 112.87 0 0 1-69.7 24 119 119 0 0 1-13.4-.8 158.83 158.83 0 0 0 86 25.2c103.2 0 159.6-85.5 159.6-159.6 0-2.4-.1-4.9-.2-7.3a114.25 114.25 0 0 0 28.1-29.1' fill='currentColor' /></svg>,
                    value: data.twitter
                  },
                  {
                    icon: 'email',
            ...
            const footerStats = data.repositoryCount || data.peopleCount || data.projectCount
              ? [
                  {
                    icon: 'class',
                    label: data.repositoryCount !== 1 ? 'Repositories' : 'Repository',
                ...
                <Flex sx={{ mb: 18, alignItems: 'center' }}>
                  ...
                  <Box>
                    ...
                    >
                      ...
                      {!!data.verified && (
                        <Text className='material-icons' sx={{ fontSize: 22, color: '#57ab5a', ml: '8px' }}>
                          verified
                        </Text>
                      )}
                    </Text>
                    ...
                    {!!headerStats && (
                      <Flex sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
                        {headerStats.map((stat, index) => !!stat.value && (
                          <Flex key={index} sx={{ alignItems: 'center', mr: 20, mb: '8px' }}>
                            <Text className='material-icons' sx={{ mr: '6px', fontSize: '16px', color: '#76767F' }}>{stat.svg || stat.icon}</Text>
                            <Text sx={{ fontSize: '12px' }}>{stat.value}</Text>
                          </Flex>
                        ))}
                      </Flex>
                    )}
                ...
                {!!data.languages && (
                  <Flex sx={{ flexWrap: 'wrap' }}>
                    {data.languages.map(({ color, language }) => (
                      <Flex key={language} sx={{ mr: 18, mb: 12, alignItems: 'center' }}>
                        <Box
                          sx={{
                            width: 16,
                            height: 16,
